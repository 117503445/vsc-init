...
func main() {
	...
	err := goutils.WriteText("/root/.code-server/User/settings.json", assets.Settings)
	if err != nil {
		log.Fatal().Err(err).Msg("write settings.json error")
	}

	err = goutils.WriteText("/root/.code-server/User/keybindings.json", assets.KeyBindings)
	if err != nil {
